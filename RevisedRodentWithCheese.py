...
'''
	added a method for each sprite that returns the type of sprite using a numerical system. 
    Obstacles are 1, Enemies are 2, Cheese is 3, Goal is 4
	this is useful in collisions for determining what object the rogue is touching. MT

    I think the code might be easier to parse if we use the type of the object instead of the numerical system,
    but the numerical system works fine. TZ
'''
...
class Enemy(pygame.sprite.Sprite):
    #Creates a block that can be collided with. MT
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
...
class Cheese(pygame.sprite.Sprite):
    ...
    def setCollected(self, answer):
        '''
            Changes self.collected to answer to indicate whether it's been collected. Somewhat obsolete now but may be useful later. TZ
        '''
        self.collected = answer

    def getCollected(self):
        '''
            Returns a Boolean for whether the rodent has the cheese. Somewhat obsolete now but may be useful later. TZ
        '''
...
'''
	...
	commented out the entire "down" portion, since we have no use for this key right now. MT
'''
'''
    I re-added some of the image stuff because I was planning to use it (didn't work out), and it helps with showing you got the cheese. TZ
'''
...
class Rogue(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, scene):
        ...
        self.x = x
        self.y = y
        self.scene = scene #MT. Mike also added the variable scene in the init
        self.contact = False
        self.jump = False
        ...
        self.win = False # MT
        #TZ
        self.font = pygame.font.SysFont("Times New Roman", 21, bold=True)
        self.icon = self.font.render("Sorry! You can't leave without the cheese!", True, (0, 0, 0))
        #My changes end here. TZ
        #set to mouse symbol. 
...
#                 self.image = self.image = pygame.image.load("actions/rodent.png").convert_alpha()
	...
	'''
		...
		modified hasCheese, dead, and win variables. MT
	'''
        #Controls collision
        self.contact = False
        for o in scene:
            if self.rect.colliderect(o):
                ...
                if (movy < 0 and o.getType() != 3 and o.getType() != 2 and o.getType() != 4):
                    ...
                    self.movy = 0
                if o.getType() == 3:
					self.icon = self.font.render("Cheese!", True, (0, 0, 0))
					self.hasCheese = True
                    #self.scene.remove(o) #My text editor is randomly fighting me on indentation for some reason, but commenting this out made it shut up, and the cheese still vanishes
                if o.getType() == 2:
                	self.dead = True
                if o.getType() == 4:
                	if self.hasCheese == True:
...
class Level(object):
    ...
    def getSprites(self):
        '''
            Returns all the sprites contained in the Group object of the level. Somewhat obselete now but may be useful later. TZ
        '''
        return self.scene
...
def getCheese(sprites):
    '''
        ...
        Written to be used with Level's getSprites method
        Somewhat obselete now but may be useful later. TZ
    '''
...
'''
	created a method that runs an individual level based on a given filename (preferably .txt) MT
'''
	...
	while True:
		...
		for x in range(0, asize[0], background_rect.w):
			for y in range(0, asize[1], background_rect.h):
                ...
                screen.blit(level.Rogue.icon, (10, 10))
		...
		if Rogue.getHasCheese():
			all_sprite.remove(level.cheese)
                level.cheese.setCollected(True)
...
'''
	main code runs the levels in sequence. MT
'''
